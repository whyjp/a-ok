...
# 7) 대시보드 (SQLite 기반 BFF + 정적 FE) 띄우기
workerctl view serve             # http://127.0.0.1:8765/ 에서 대시보드 + API 제공
workerctl dashboard              # 동일 명령의 짧은 별칭
workerctl view serve --open      # 띄운 뒤 기본 브라우저로 열기
workerctl view serve --port 9000
workerctl view serve --db D:/work-github/.worker-control/worker-control.sqlite3

# 8) (선택) 오프라인 첨부용 단일 파일 스냅샷 — 레거시
workerctl view html --legacy     # D:/work-github/.worker-control/dashboard.html 작성
workerctl view html --legacy --open

# 9) Hermes 시작 시 대시보드 상시 실행 + 코드 변경 시 자동 재시작
workerctl dashboard-daemon --log D:/work-github/.worker-control/dashboard.log

# 10) Hermes cron (30 분) — 살아있는 세션이 있으면 Telegram 으로 snapshot
